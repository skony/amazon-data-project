...
public class ProductReviewsRequester extends AbstractApiRequester {
    ...
    public Map<String, Integer> runRequesting(WebTarget baseTarget) {
        ...
        Stream.of(DatabasePath.values())
                .forEach(dbPath -> {
                    WebTarget reviewTarget = baseTarget.path(dbPath.getPath() + "/review");
    ...
    private int request(WebTarget target) {
        return (int) REQUESTED_PRODUCT_IDS.stream()
                .map(product -> {
                    WebTarget productTarget = target.queryParam("product", product);
